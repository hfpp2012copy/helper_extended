...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/hfpp2012copy/helper_extended/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("helper_extended.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in helper_extended.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "helper_extended.gemspec", /ruby_version = .*">= (.*)"/ => lowest
